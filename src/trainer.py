...
def load_checkpoint(
    ...
    """
    Load checkpoint with support for architecture changes.
    
    Args:
        model: Model instance
        optimizer: Optimizer instance  
        checkpoint_dir: Directory containing checkpoints
        advanced: If True, allows loading with different architecture
                 (useful when switching triplet <-> arcface)
    """
    ...
    if state_dict_g:
        if advanced:
            # Allow loading models with different loss architectures
            model_dict = model.state_dict()
            
            # Filter: only load weights that exist in current model
            valid_dict = {
                k: v for k, v in state_dict_g["generator"].items() 
            ...
            # Log what wasn't loaded (new parameters)
            missing = set(model_dict.keys()) - set(valid_dict.keys())
            if missing:
                logging.warning("These parameters were randomly initialized:")
                for k in sorted(missing):
                    if "_triplet_loss" in k:
                        logging.warning("  %s (Triplet loss parameter)", k)
                    elif "_arcface_loss" in k:
                        logging.warning("  %s (ArcFace loss parameter)", k)
                    else:
                        logging.warning("  %s", k)
            
            # Log what was in checkpoint but not loaded (old parameters)
            extra = set(state_dict_g["generator"].keys()) - set(model_dict.keys())
            if extra:
                logging.info("These checkpoint parameters were not used:")
                for k in sorted(extra):
                    if "_triplet_loss" in k:
                        logging.info("  %s (old Triplet loss parameter)", k)
                    elif "_arcface_loss" in k:
                        logging.info("  %s (old ArcFace loss parameter)", k)
                    else:
                        logging.info("  %s", k)
        else:
            # Standard loading - architectures must match exactly
    ...
    if state_dict_do is None:
        ...
        # Try to load optimizer state, but don't fail if architectures don't match
        if optimizer is not None:
            if advanced:
                # When using advanced mode (architecture changed), skip optimizer loading
                # The optimizer will be reset with new learning rate anyway
                logging.warning("Advanced mode: Skipping optimizer state (will be reset)")
            else:
                try:
                    optimizer.load_state_dict(state_dict_do["optim_g"])
                except (ValueError, KeyError) as e:
                    logging.warning(f"Could not load optimizer state: {e}")
                    logging.warning("Optimizer will use fresh state")
